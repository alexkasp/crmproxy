...
public:
	...
	const string& getPickupNum() const;
...
{
	string parse_pickup(string callid, string pickupcallid, string callednum, string answernum);
	string parse_hangup(string callid, string peername, string uidcode);
	...
	string parse_initcall(string src,string dst,string uid,string timestamp,string callid,string recordfile,string usecrm,
	    string uidcode,string treeid,string channel,string roistat,string roistatphone,string roistatmarket,string roistatsource,string xcallerid,string calltype,string transfer,string from_number, string outLine);
